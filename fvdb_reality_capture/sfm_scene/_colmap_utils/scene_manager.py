    ...
    def _load_points3D_txt(self, input_file):
        ...
        with open(input_file, "r") as f:
            for line in iter(lambda: f.readline().strip(), ""):
                ...
                pts_i = [np.float64(x) for x in data[1:4]]
                pts_rgb_i = [np.uint8(x) for x in data[4:7]]
                error_i = np.float64(data[7])
                self.points3D.append(pts_i)
                self.point3D_colors.append(pts_rgb_i)
                self.point3D_errors.append(error_i)

                # load (image id, point2D idx) pairs
                point_ids = np.array([np.uint32(x) for x in data[8:]])
                self.point3D_id_to_images[point3D_id] = point_ids.reshape(-1, 2)
