...
- Moved the high-level `GaussianSplat3d`, `ProjectedGaussianSplats`, and `evaluate_spherical_harmonics` APIs from `fvdb` to `fvdb_reality_capture`. This is a breaking import-path change; the low-level compiled kernels and supporting tensor types remain in fVDB-core.
